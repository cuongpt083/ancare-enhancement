...
FLOW = [
  ...
  ('coach','CONS-06_chot_goi','Chốt gói'),
  ...
  ('customer','HLTH-01_dong_ho_sinh_hoc','Đồng hồ sinh học'),
  ('customer','HLTH-04_bua_an','Bữa ăn KH'),
  ('customer','HLTH-05_chat','Chat KH'),
  ('customer','HLTH-07_bao_cao','Báo cáo KH'),
  ('customer','DEV-02_dao_tao','Đào tạo KH'),
]

proto_dir = '/Users/cuongpt/Workspaces/ancare-enhancement/docs/04-prototypes'
os.makedirs(f'{proto_dir}/_assets', exist_ok=True)
os.makedirs(f'{proto_dir}/coach', exist_ok=True)
os.makedirs(f'{proto_dir}/customer', exist_ok=True)

# proto-nav.css
open(f'{proto_dir}/_assets/_proto-nav.css', 'w', encoding='utf-8').write('''@import url('https://fonts.googleapis.com/css2?family=Be+Vietnam+Pro:wght@400;500;600;700&display=swap');
...
'''
    open(f'{proto_dir}/{role}/{name}.html', 'w', encoding='utf-8').write(html)
...
<div class="flow">
  <h2>🟢 Luồng Tư vấn 15 phút (HLV) <span class="badge">11 màn</span></h2>
  <div class="grid">
'''
for role, name, title in FLOW[:11]:
    index += f'    <a class="card" href="coach/{name}.html"><div class="id">{i+1}/{len(FLOW)}</div><div class="nm">{title}</div><div class="tpl">T1</div></a>\n'
index += '''  </div>
</div>

<div class="flow">
  <h2>📋 Chăm sóc KH (HLV) <span class="badge">3 màn</span></h2>
  <div class="grid">
'''
for role, name, title in FLOW[11:15]:
...
<div class="flow kh">
  <h2>🌿 Trải nghiệm Khách hàng (KH) <span class="badge">7 màn</span></h2>
  <div class="grid">
'''
for role, name, title in FLOW[15:]:
...
<div class="note">
  ...
  <strong>Luồng đầy đủ HLV:</strong> Đăng nhập → Trang chủ → DS KH → Thêm mới KH → Chân dung → Tanita → Phân tích → Xem lộ trình → Chốt gói → Tạo TK → Bữa ăn.<br>
  <strong>Tổng:</strong> 22 mockup + 22 prototype. Wireframe đã rebuild theo design system mới (không emoji, SVG Tabler, semantic tokens, 8dp spacing).
</div>
</div></body></html>
'''
open(f'{proto_dir}/index.html', 'w', encoding='utf-8').write(index)
