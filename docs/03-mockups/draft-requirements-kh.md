...
- Màn hình trang chủ: hiển thị tổng quát các thông tin của KH. Các thông tin chính bao gồm:
    - Thông tin cơ bản của KH: ảnh đại diện (avatar) của KH, lời chào mừng, chuông thông báo (tùy chọn để bật/tắt nhận thông báo), gói giải pháp KH đã mua (nếu KH có mua gói giải pháp)
    - Các nút check-in thực hiện các nhiệm vụ trong ngày. Khi bấm vào nút check in, ứng dụng sẽ hiển thị màn hình "Check-in" để KH thực hiện các nhiệm vụ trong ngày. 
    - Dashboard tóm tắt sức khỏe toàn diện (theo dạng đồng hồ sinh học): chấm điểm các nhiệm vụ hàng ngày.
    - Báo cáo tổng quát về tiến trình trải nghiệm gói Giải pháp của khách hàng theo dạng biểu đồ line-chart nhằm thể hiện xu hướng cải thiện sức khỏe, thể chất của cá nhân KH.
    - Bảng thống kê chỉ số cơ thể: hiển thị như sau:
        + các cột: Tên chỉ số, Mục tiêu, Ngày đầu (chỉ số ngày đầu tiên), Ngày gần nhất (chỉ số ngày gần nhất), Thay đổi. Nếu chỉ số nằm trong ngưỡng mục tiêu thì hiển thị giá trị với màu xanh, nằm ngoài ngưỡng mục tiêu thì hiển thị với màu đỏ. Cột "Thay đổi" hiển thị ký hiệu mũi tên lên (màu xanh) nếu chỉ số cải thiện so với ngày đầu và mũi tên xuống (màu đỏ) nếu chỉ số xấu đi so với ngày đầu.
        + các dòng: các chỉ số cơ bản, bao gồm: Cân nặng, Mỡ cơ thể, Mỡ nội tạng, Cơ bắp, Xương, Nước, Tuổi sinh học, Trao đổi chất cơ bản.
- Màn hình Báo cáo: hiển thị các báo cáo bao gồm:
...
- Card 1: Thông tin chỉ số cơ thể. Card này hiển thị các thông tin sau: Cân nặng (kg), Mỡ cơ thể (%), Mỡ nội tạng (%), Cơ bắp (kg), Xương (kg), Nước (%), Tuổi sinh học, Trao đổi chất cơ bản (kcal). Các thông tin này tổ chức dưới dạng một bảng thông tin với 3 cột. Phía dưới bảng này là một nút bấm dạng link cho phép bấm vào để nhập lại các thông số này (nếu như KH muốn nhập lại, mặc định là đã nhập từ sáng), khi nhập lại thì dữ liệu sẽ được ghi đè dữ liệu đã có, nếu chưa có thì được thêm mới.
- Card 2: cThực đơn hôm nay. Card này bao gồm các mục nhỏ sau (kèm theo icon minh họa tương ứng, danh sách các mục này tùy theo số bữa KH đã đăng ký trong gói giải pháp đang sử dụng, ví dụ có gói 3 bữa/ngày, có gói 5 bữa/ngày)
    - Bữa sáng
    - Ăn nhẹ buổi sáng
    - Bữa trưa
    - Ăn nhẹ buổi chiều
    ...
    - Nút chụp ảnh: cho phép chụp ảnh món ăn để AI đánh giá. Khi bấm vào nút này ứng dụng mở pop-up để KH chụp ảnh món ăn (có thể chụp ảnh hoặc upload ảnh từ thư viện ảnh có sẵn). Sau khi chụp ảnh xong, ứng dụng hiển thị ảnh và AI sẽ đánh giá cấu trúc bữa ăn và hiển thị dưới dạng Calo và gam Đạm đối với 3 nhóm: Đạm - Tinh bột - Chất béo. Đây là các giá trị ước tính, KH có thể chỉnh sửa lại để phù hợp với thực tế. Sau khi chỉnh xong KH bấm nút “Lưu” để lưu lại thông tin. Nếu KH có sử dụng các sản phẩm hỗ trợ, ứng dụng sẽ hiển thị thêm mục bổ sung thông tin sử dụng các sản phẩm hỗ trợ này (ví dụ: số viên, số ml...).
...
- Card 3: check-in hoạt động dinh dưỡng và thể chất khác như Uống nước, Giấc ngủ, Thể thao:
...
- Card 5: check-in kiến thức. Card này bao gồm các mục nhỏ sau:
